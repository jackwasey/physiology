...
#' creatinine concentration (including pregnancy, seruios co-morbid conditions,
...
#'     * If eGFR,MDRD is estimated < 60 mL/min/1.73 m^2 and eGFR,CKD-EPI < 60, return eGFR,MDRD.
#'     * If eGFR,MDRD is estimated >= 60 mL/min/1.73 m^2 and eGFR,CKD-EPI > 60, return eGFR,CKD-EPI.
#'     * Otherwise, return the average of eGFR,MDRD and eGFR,CKD-EPI.
#' 
#' If an IDMS-calibrated assay is used (`idms_assay = TRUE`), the MDRD equation
#' will be corrected for the assay by approximately 6%, the CKD-EPI equation is
#' only validated for use with IDMS-calibrated assays, and the Cockroft-Gault is
#' not calibrated for use with an IDMS-calibrated assay.  Most labs follow the
#' National Kidney Disease Education Program (NKDEP) recommendation to use an
#' IDMS-calibrated assay, so by default `idms_assay = TRUE`.
#'
...
egfr <- function(scr_uM, age_y, height_m, male, black, ...) {
  ...
  if (any(mask_adult)) {
    ...
    if (any(use_both)) {
      warning(sum(use_both), " values recommend using both MDRD and CKD-EPI equations.  Averaging the values.")
      mdrd[use_both] <- rowMeans(cbind(mdrd[use_both], ckdepi[use_both]))
    }
    if (any(use_neither)) {
      warning(sum(use_neither), " values recommend using neither MDRD nor CKD-EPI equations.  Averaging the values.")
      mdrd[use_neither] <- rowMeans(cbind(mdrd[use_neither], ckdepi[use_neither]))
    }
...
egfr_cockroft_gault <- function(scr_uM, age_y, weight_kg, male, idms_assay=TRUE, ...) {
  ...
  valid_age(age_y, ...)
  if (idms_assay) {
    warning("The Cockroft-Gault equation for eGFR is not calibrated for use with an IDMS-calibrated assay.  Interpret results with caution.")
  }
  (140-age_y)*
    weight_kg*
    (0.85 + 0.15*male)/
...
#' Med. 2006 Aug 15;145(4):247-54.
#' @export
egfr_mdrd <- function(scr_uM, age_y, male, black, idms_assay=TRUE, ..., warn_ckdepi_preferred=TRUE) {
  stopifnot(length(scr_uM) == length(age_y))
  stopifnot(length(scr_uM) == length(male))
  stopifnot(length(scr_uM) == length(black))
  ...
  if (any(ret > 60) & warn_ckdepi_preferred) {
    warning("The MDRD equation for eGFR is not recommended for values above 60 mL/min/1.73m^2 (CKD-EPI is recommended).  Use results above 60 with caution.")
  }
  ret
}
...
#' @export
egfr_ckdepi <- function(scr_uM, age_y, male, black, idms_assay=TRUE, ..., warn_mdrd_preferred=TRUE) {
  stopifnot(length(scr_uM) == length(age_y))
  stopifnot(length(scr_uM) == length(male))
  stopifnot(length(scr_uM) == length(black))
  stopifnot(all(male %in% c(TRUE, FALSE)))
  ...
  valid_age_adult(age_y, ...)
  if (any(age_y < 18)) {
    warning("The CKD-EPI equation for eGFR is not recommended for ages below 18 years.")
  }
  if (!idms_assay) {
    warning("The CKD-EPI equation is only designed for use with an IDMS-calibrated serum creatinine assay.")
  }
  ...
  if (any(ret < 60) & warn_mdrd_preferred) {
    warning("The CKD-EPI equation for eGFR is not recommended for values below 60 mL/min/1.73m^2 (MDRD is recommended).  Use results below 60 with caution.")
  }
  ret
}

#' @describeIn egfr The Bedside Schwartz equation for eGFR (for children, age < 18 years).
#' @references
#' \url{https://www.niddk.nih.gov/health-information/communication-programs/nkdep/laboratory-evaluation/glomerular-filtration-rate-calculators/children-si-units}
#' 
...
egfr_bedside_schwartz <- function(scr_uM, height_m, idms_assay=TRUE, ...) {
  ...
  valid_height(height_m, ...)
  if (!idms_assay) {
    warning("The Bedside-Schwartz equation is calibrated for use with an IDMS-traceable assay.  eGFR from a non-IDMS-calibrated assay will be biased.")
  }
