...
#' @describeIn bsa Deprecated, please use `bsa_mosteller()`
#' @export
bsa_adult <- function(height_m, weight_kg, ...) {
  .Deprecated("bsa_mosteller")
...
#' @export
bsa_dubois_dubois <- bsa_simple(coefficient = 0.007184, w_exp = 0.425, h_exp = 0.725)
...
#' @export
bsa_haycock <- bsa_simple(coefficient = 0.024265, w_exp = 0.5378, h_exp = 0.3964)

#' @describeIn bsa Gehan and George formula
#' @references Gehan EA, George SL, Cancer Chemother Rep 1970, 54:225-235
#' @export
bsa_gehan_george <- bsa_simple(coefficient = 0.0235, w_exp = 0.51456, h_exp = 0.42246)
...
bsa_boyd <- function(height_m, weight_kg, ...) {
  ...
  valid_height_adult(height_m, ...)
  valid_weight_adult(weight_kg, ...)
  0.03330 * (weight_kg^(0.6157 - 0.0188 * log10(weight_kg))) * ((height_m * 100)^0.3)
}
...
#' @describeIn bsa Takahira formula (a variant of Du Bois that could be used for Japanese individuals; Fujimoto may be preferred)
#' @references Fujimoto S, Watanabe T, Sakamoto A, Yukawa K, Morimoto K. Studies
#'   on the physical surface area of Japanese. 18. Calculation formulae in three
#'   stages over all ages. Nippon Eiseigaku Zasshi 1968;5:443–50.
#' @export
...
#' @export
bsa_shuter_aslani <- bsa_simple(coefficient = 0.00949, w_exp = 0.441, h_exp = 0.655)

bsa_schlich_female <- bsa_simple(coefficient = 0.000975482, w_exp = 0.46, h_exp = 1.08)
bsa_schlich_male <- bsa_simple(coefficient = 0.000579479, w_exp = 0.38, h_exp = 1.24)
