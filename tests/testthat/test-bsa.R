...
test_that("body surface area, deprecated", {
  expect_equal(
    expect_warning(bsa_adult(height_m = 2, weight_kg = 72)),
    2
  )
  expect_equal(
    expect_warning(bsa_adult(height_m = c(NA, 2), weight_kg = c(70, 72))),
    c(NA_real_, 2)
  )
  # bsa_adult is replaced with bsa_mosteller
  expect_equal(
    expect_warning(bsa_adult(height_m = c(NA, 2), weight_kg = c(70, 72))),
...
test_that("body surface area test interface fuzzing", {
  ...
  for (current_fun in bsa_functions) {
    ...
    if (!("male" %in% names(formals(current_fun)))) {
      ...
      expect_false(
        do.call(current_fun, tmp_args_male) == do.call(current_fun, tmp_args_female)
      )
    }
