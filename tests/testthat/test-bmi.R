...
test_that("ideal_weight_adultAdult", {
  ...
  expect_warning(ideal_weight_adult(height_m = 0, male = TRUE, do.warn = TRUE))
  # should warn when height is out of validated range of the formula
  expect_warning(ideal_weight_adult(height_m = -1, male = TRUE, do.warn = TRUE))
  # should warn when height is out of validated range of the formula
  expect_warning(ideal_weight_adult(height_m = 3, male = TRUE, do.warn = TRUE))
  # should warn when height is out of validated range of the formula
...
test_that("body surface area", {
  ...
  expect_equal(bsa_adult(height_m = c(NA, 2), weight_kg = c(70, 72)), c(NA_real_, 2))
})
...
test_that("blood vol Lemmens not obese", {
  ...
  expect_error(blood_vol_Lemmens_non_obese(weight_kg = 50, age = 50, male = "M"))

  expect_error(blood_vol_Lemmens_non_obese(weight_kg = 50, age = 50, male = c(TRUE, FALSE)))
  expect_error(blood_vol_Lemmens_non_obese(weight_kg = 50, age = c(50, 60), male = FALSE))
  expect_error(blood_vol_Lemmens_non_obese(weight_kg = c(50, 75), age = 50, male = FALSE))
...
test_that("height, weight funcs invalid input", {
  ...
  for (f in funs) {
    ...
    #mismatch length
    expect_error(do.call(f, list(height_m = c(1.5, 2), weight_kg = 40)), info = f)
    expect_error(do.call(f, list(height_m = 2, weight_kg = c(40, 50))), info = f)
    expect_error(do.call(f, list(height_m = c(1.5, 2), weight_kg = NULL)), info = f)
    expect_error(do.call(f, list(height_m = NULL, weight_kg = c(40, 50))), info = f)
    expect_error(do.call(f, list(height_m = c(1.5, NA), weight_kg = 40)), info = f)
    expect_error(do.call(f, list(height_m = 2, weight_kg = c(NA, 50))), info = f)
    expect_error(do.call(f, list(height_m = NULL, weight_kg = 40)), info = f)
    expect_error(do.call(f, list(height_m = 2, weight_kg = NULL)), info = f)
    ...
    expect_error(do.call(f, list(height_m = -1, weight_kg = 50, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m =  0, weight_kg = 50, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, weight_kg = -50, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, weight_kg =   0, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 8, weight_kg = 50, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, weight_kg = 5000, do.stop = TRUE)), info = f)
  }
})
...
test_that("height, gender funcs invalid input", {
  ...
  for (f in funs) {
    ...
    # tighter rules or other arguments required, so we can only look for errors
    # here, if we test all ht/wt functions together.
    expect_error(do.call(f, list(height_m = -1, male = T, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m =  0, male = T, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, male = "m", do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, male = 2L, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 1.5, male = -1, do.stop = TRUE)), info = f)
    expect_error(do.call(f, list(height_m = 8, male = T, do.stop = TRUE)), info = f)
  }
})
